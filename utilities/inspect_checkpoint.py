# Modified by Jiong Zhu (jiongzhu@umich.edu) to allow loaded tensors to be
# returned in a dictionary. Tested with TensorFlow 1.8. 
# =============================================================================
...
def load_tensors_in_checkpoint_file(file_name, tensor_name, all_tensors,
                                     all_tensor_names=False):
  """Load tensors in a checkpoint file.
  ...
  try:
    ...
    if all_tensors or all_tensor_names:
      ...
      for key in sorted(var_to_shape_map):
        if all_tensors:
          return_dict[key] = reader.get_tensor(key)
    elif not tensor_name:
      print(reader.debug_string().decode("utf-8"))
    else:
      return_dict[tensor_name] = reader.get_tensor(tensor_name)
    return return_dict
  except Exception as e:  # pylint: disable=broad-except
    print(str(e))
    if "corrupted compressed block contents" in str(e):
